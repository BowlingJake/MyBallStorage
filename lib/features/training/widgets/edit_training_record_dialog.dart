...
typedef OnRecordUpdatedCallback = void Function(TrainingFormState record);

void showEditTrainingRecordDialog(
  BuildContext context, {
  required TrainingDaySummary summary,
  required OnRecordUpdatedCallback onRecordUpdated,
}) {
  showDialog(
    context: context,
...
// 編輯訓練記錄彈窗 - 使用 Riverpod 狀態管理
class EditTrainingRecordDialog extends ConsumerStatefulWidget {
  ...
  final OnRecordUpdatedCallback onRecordUpdated;

  @override
  ConsumerState<EditTrainingRecordDialog> createState() =>
      _EditTrainingRecordDialogState();
}

class _EditTrainingRecordDialogState extends ConsumerState<EditTrainingRecordDialog>
  ...
  Future<void> _selectDate(BuildContext context) async {
    final state = ref.read(trainingFormProvider(widget.summary));
    final notifier = ref.read(trainingFormProvider(widget.summary).notifier);
              ...
              children: [
                ...
                Consumer(
                  builder: (context, ref, child) {
                    final state = ref.watch(trainingFormProvider(widget.summary));
                    return Text(
        ...
        builder: (context, ref, child) {
          ...
          return Row(
            children: [
              ...
              Expanded(
                child: AppStandardButton(
                  text: state.currentStep == 1 ? 'Update Session' : 'Next',
                  icon: state.currentStep == 1 ? Icons.check : Icons.arrow_forward,
                  onPressed:
