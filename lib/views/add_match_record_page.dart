...
class _AddMatchRecordPageState extends State<AddMatchRecordPage> {
  ...
  int _getGamesPerSession() {
    if (widget.tournamentType == TournamentType.open &&
        widget.openFormat == OpenTournamentFormat.mq) {
      return widget.mqGamesPerSession ??
          6; // Use MQ setting, default to 6 if null (shouldn't happen with validation)
    } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('記錄比賽分數 - ${widget.tournamentName}')),
      body: Column(
        // Use Column to place button at the bottom
        children: [
          Expanded(
            ...
            child: ListView(
              ...
              children: <Widget>[
                ...
                Row(
                  ...
                  children: [
                    Expanded(
                      // Allow location to expand if needed
                      child: Text(
                ...
                Text(
                  '類型: ${widget.tournamentType == TournamentType.open ? '公開賽' : '錦標賽'}${widget.tournamentType == TournamentType.open && widget.openFormat != null ? ' (${widget.openFormat == OpenTournamentFormat.mq ? 'MQ' : '經典賽'})' : ''}', // Add format if applicable
                  style: Theme.of(context).textTheme.titleMedium,
                ),
                const SizedBox(height: 16),

                // Display Selected Balls (using the GridView)
                Text('使用球具:', style: Theme.of(context).textTheme.titleMedium),
                ...
                GridView.builder(
                  ...
                  gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                    ...
                    mainAxisSpacing:
                        18, // Adjusted spacing between rows by adding 8px
                    childAspectRatio:
                        2.5, // Adjust aspect ratio for card look (width > height)
                  ),
                ...
                Text('比賽成績:', style: Theme.of(context).textTheme.titleLarge),
                const SizedBox(height: 16),
                // TODO: Display recorded scores here later (e.g., from TournamentViewModel)
                const Center(
                  child: Text('尚未登錄成績', style: TextStyle(color: Colors.grey)),
                ),
              ],
            ),
          ),
...
Widget _buildArsenalStyleBallCard(BuildContext context, BowlingBall ball) {
  ...
  final smallStyle = baseStyle.copyWith(
    fontSize: 14,
    color: hasSpecialBg ? Colors.white70 : Colors.grey,
  );
