  ...
  void _saveCustomBall() {
    if (_formKey.currentState!.validate()) {
      ...
      final newBall = BowlingBall(
        ...
        core:
            _coreController.text
                .trim(), // Consider simplifying core input later
        coverstockname: _coverstockController.text.trim(), // Use input for name
        coverstockcategory:
            _coverstockController.text.trim(), // Use input for category for now
        ...
        releaseDate:
            _releaseDateController.text.trim(), // Keep as string for now
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('"${newBall.ball}" 已成功新增至我的球櫃')));
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: <Widget>[
            ...
            _buildTextField(
              _coverstockController,
              '球面材質名稱 (Coverstock Name)',
              icon: Icons.texture,
            ),
            _buildTextField(
              _rgController,
            ...
            _buildTextField(
              ...
              keyboard: const TextInputType.numberWithOptions(decimal: true),
              isRequired: false,
            ), // MB Diff might not always exist
            _buildTextField(
